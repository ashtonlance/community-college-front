...
export default function NewsTaxonomyPage({
  data,
  loading,
...
NewsTaxonomyPage.variables = (props, ctx) => {
  console.log({ props })
...
NewsTaxonomyPage.query = gql`
  ...
  query Page($databaseId: ID!, $asPreview: Boolean = false, $slug: [String]) {
    ...
    newsItems(
      ...
      where: {
        taxQuery: {
          taxArray: { operator: EXISTS, taxonomy: NEWSTAG, terms: $slug }
        }
        orderby: { field: DATE, order: DESC }
      }
    ) {
